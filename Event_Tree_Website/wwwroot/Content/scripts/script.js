$(document).ready(function () {
    $("#bars").click(function () {
        ...
        //    Đoạn mã này được sử dụng để thêm một hành động click vào các phần tử có lớp là
        //"go-to-top".Khi một phần tử với lớp này được click, nó sẽ chạy một hàm để thực hiện một hiệu 
        //ứng cuộn lên đầu trang.Điều này thường được sử dụng để cung cấp một cách thuận tiện cho người 
        //dùng để quay lại đầu trang của trang web một cách nhanh chóng.
    ...
    //select cat
    $(".select-cat").click((e) => {
        let classSelect = e.target.dataset.select;
        let classParentSelect = e.target.dataset.parent;

        let selectParent = "select-" + classParentSelect.split("-")[1];
        $("." + selectParent).removeClass("activeColor");
        e.target.classList.add("activeColor");
        console.log(selectParent, e.target);

        $("." + classParentSelect).removeClass("active");
        $("." + classSelect + "." + classParentSelect).addClass("active");
    })
